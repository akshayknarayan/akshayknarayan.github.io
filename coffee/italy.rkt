...
@(define (italy make-cafe)
  @div{
    @p[class: "ul-aligned-para"]{Note: Italian coffee culture is somewhat different than the other places on this page. For example, Italians don't drink heated-milk drinks such as cappucinos after 11:00. 
    ...
    @p[class: "ul-aligned-para"]{As a result, my recommendation is a cappuccino in the morning and a cafe macchiato (espresso with a bit of foamy milk) in the afternoon. 
    ...
    @p[class: "ul-aligned-para"]{Also, I have only listed the places that especially stood out to me in some way. It is in almost all cases perfectly fine to walk into an arbitrary "caffetteria" for coffee.
    }
            
    @p[class: "ul-aligned-para"]{Now, my actual recommendations:}
